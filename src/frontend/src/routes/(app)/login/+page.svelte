<script lang="ts">
  ...
  import { translations } from "$lib/data/translations"; // Translation object
  import { selectedLanguage } from "$lib/data/stores/languageStore"; // Language store
  import LanguageSwitcher from "$lib/components/LanguageSwitcher.svelte"; // Language switcher component
...
<svelte:head>
  <title>{translations[lang].signIn} - DeComm</title>
...
<div class="grid grid-cols-12 min-h-screen max-h-screen w-full z-50">
  ...
  <div class="col-span-12 lg:col-span-6 w-full h-full flex justify-center items-center">
    <div class="absolute right-10 top-10 flex gap-4 items-center">
      ...
      <LanguageSwitcher />
      <!-- Register Button -->
      <Button variant="outline" on:click={registerCheck}>{translations[lang].registerButton}</Button>
    </div>
    <div class="block text-center">
      <h1 class="font-semibold text-2xl mb-2">{translations[lang].signIn}</h1>
      <p class="opacity-75">{translations[lang].enterName}</p>
